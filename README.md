# ASCEND'26 information site

Static public website for the ASCEND'26 Postgraduate Colloquium organised by Universiti Teknologi MARA Cawangan Perlis.

## Local preview

The site has no build step or external dependencies. Open index.html directly, or serve the directory with:

    python -m http.server 8080

Then visit http://localhost:8080.

## Publishing

GitHub Pages serves the repository root from the main branch. The .nojekyll marker prevents Jekyll processing.

## Content status

The source poster is a draft. Details explicitly labelled **TBC** or **Draft** must be confirmed before the site is treated as the final public announcement, especially:

- the MYR 30 / USD 8 fee category;
- venue and participation arrangements;
- registration/submission system URL; and
- final wording of the research fields.

The draft poster is intentionally not published or linked from this repository.
